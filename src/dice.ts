...
export class DiceRoller {
   ...
   */
  static rollStats(): { scores: number[], details: string[] } {
    const scores: number[] = [];
   ...
   */
  /**
   * Roll multiple iterations of the same dice expression
   * e.g., rollRepeated(3, "2d20+5") rolls 2d20+5 three times
   */
  static rollRepeated(iterations: number, expression: string): string {
    if (iterations < 1 || iterations > 20) {
      return "❌ Iterations must be between 1 and 20";
    }
    ...
    let output = results.map((r, i) => `**Roll ${i + 1}:** ${r.details}`).join("\n");

    if (totals.length > 0) {
      const sum = totals.reduce((a, b) => a + b, 0);
      const avg = (sum / totals.length).toFixed(1);
      output += `\n\n**Summary:** ${totals.join(", ")}\n**Total Sum:** ${sum} | **Average:** ${avg}`;
    }
  ...
  static rollThresholdStat(): { total: number, rolls: number[] } {
    let rolls = Array.from({ length: 4 }, () => {
      ...
      if (roll <= 2) roll = Math.floor(Math.random() * 6) + 1; // Reroll
      return roll;
    });
