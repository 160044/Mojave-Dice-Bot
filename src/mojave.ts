...
import { StatusTracker, DND_CONDITIONS } from './status';
...
persistence.startAutoSave(
  () => initiativeTracker.exportData(),
  () => statusTracker.exportData()
);
 ...
 */
export function initializeMojave(): void {
 ...
 * Route commands
 */
async function routeCommand(channelId: ChannelGuid, command: string, args: string[], userId: string): Promise<void> {
  // Utility
  if (command === 'ping') {
  ...
  if (command === 'rr' || command === 'repeat') {
    // !rr N XdY - Roll XdY N times
    if (args.length < 2) {
      await sendMessage(channelId, 'Usage: `!rr <iterations> <dice>` - Example: `!rr 3 2d20+5` rolls 2d20+5 three times');
      return;
    }
    ...
    if (isNaN(iterations)) {
      await sendMessage(channelId, '❌ Iterations must be a number');
      return;
    }
    
    const result = DiceRoller.rollRepeated(iterations, expression);
    await sendMessage(channelId, result);
    return;
  }
 ...
 */
async function handleRoll(channelId: ChannelGuid, args: string[]): Promise<void> {
  if (args.length === 0) {
  ...
  if (result.criticalSuccess) response += '\n✨ **CRITICAL SUCCESS!**';
  if (result.criticalFailure) response += '\n💀 **CRITICAL FAILURE!**';
 ...
 * Handle advantage
 */
async function handleAdvantage(channelId: ChannelGuid, args: string[]): Promise<void> {
  const result = DiceRoller.rollAdvantage();
  let modifier = 0;
  ...
  let response = `🎲 **${result.expression}**`;
  if (modifier !== 0) response += ` ${modifier > 0 ? '+' : ''}${modifier}`;
  response += `\n${result.details}`;
 ...
 * Handle disadvantage
 */
async function handleDisadvantage(channelId: ChannelGuid, args: string[]): Promise<void> {
  const result = DiceRoller.rollDisadvantage();
  let modifier = 0;
 ...
 */
async function handleStats(channelId: ChannelGuid): Promise<void> {
  const { scores, details } = DiceRoller.rollStats();
 ...
 */
async function handleThreshold(channelId: ChannelGuid): Promise<void> {
  ...
  for (let set = 1; set <= 3; set++) {
    ...
    for (let i = 0; i < 6; i++) {
      const { total, rolls } = DiceRoller.rollThresholdStat();
      stats.push(total);
 ...
 */
async function handleDragonAgeThreshold(channelId: ChannelGuid): Promise<void> {
 ...
 * Get help message
 */
function getHelpMessage(category?: string): string {
  if (!category) {
...
**Repeat Rolls:**
\`!rr <n> <dice>\` - Roll dice n times
Examples: \`!rr 3 2d20+5\`, \`!rr 5 d20\`
...
\`!rr <n> <dice>\` - Repeat rolls (e.g., \`!rr 3 2d20+5\`)
 ...
 * Send message
 */
async function sendMessage(channelId: ChannelGuid, content: string): Promise<void> {
  try {
    // Root may have message size limits - split if needed
    ...
    if (content.length > MAX_LENGTH) {
      ...
      for (const line of lines) {
        if ((currentChunk + line + '\n').length > MAX_LENGTH) {
          ...
          currentChunk = line.length > MAX_LENGTH ? line.substring(0, MAX_LENGTH - 3) + '...' : line + '\n';
        } else {
          currentChunk += line + '\n';
        }
      }
      ...
      for (let i = 0; i < chunks.length; i++) {
        await rootServer.community.channelMessages.create({
          channelId,
          content: chunks[i].trim() + (i < chunks.length - 1 ? '\n\n*(continued...)*' : '')
        });
        if (i < chunks.length - 1) await new Promise(r => setTimeout(r, 100));
      }
    } else {
      await rootServer.community.channelMessages.create({ channelId, content });
    }
  } catch (error) {
    console.error('❌ Failed to send message:', error);
    console.error('Message length:', content.length, 'Preview:', content.substring(0, 100));
    try {
      await rootServer.community.channelMessages.create({
        channelId,
        content: '❌ Error: Response too large or failed. Try simpler command.'
      });
    } catch (e) {
...
async function handleInitShow(channelId: ChannelGuid): Promise<void> {
  const display = initiativeTracker.getDisplay(channelId, false, statusTracker);
  await sendMessage(channelId, display);
}

async function handleInitAdd(channelId: ChannelGuid, args: string[], userId: string): Promise<void> {
  if (args.length < 2) {
...
async function handleInitHidden(channelId: ChannelGuid, args: string[], userId: string): Promise<void> {
  if (args.length < 2) {
...
async function handleInitNext(channelId: ChannelGuid): Promise<void> {
  ...
  const session = (initiativeTracker as any).sessions.get(channelId);
...
async function handleInitPrev(channelId: ChannelGuid): Promise<void> {
...
async function handleInitDamage(channelId: ChannelGuid, args: string[]): Promise<void> {
  if (args.length < 2) {
...
async function handleInitHeal(channelId: ChannelGuid, args: string[]): Promise<void> {
  if (args.length < 2) {
...
async function handleInitRemove(channelId: ChannelGuid, args: string[]): Promise<void> {
  if (args.length === 0) {
...
async function handleInitClear(channelId: ChannelGuid): Promise<void> {
...
async function handleStatusShow(channelId: ChannelGuid, args: string[]): Promise<void> {
  if (args.length === 0) {
    ...
    if (DND_CONDITIONS[input]) {
      const condition = DND_CONDITIONS[input];
...
async function handleStatusAdd(channelId: ChannelGuid, args: string[]): Promise<void> {
  if (args.length < 2) {
...
async function handleStatusRemove(channelId: ChannelGuid, args: string[]): Promise<void> {
  if (args.length === 0) {
...
async function handleStatusTick(channelId: ChannelGuid): Promise<void> {
  const expired = statusTracker.tickDurations(channelId);
...
async function handleStatusClear(channelId: ChannelGuid): Promise<void> {
  statusTracker.clearChannel(channelId);
  await sendMessage(channelId, '✅ All status effects cleared!');
}

async function handleConditionsList(channelId: ChannelGuid): Promise<void> {
...
async function handleHelp(channelId: ChannelGuid, args: string[]): Promise<void> {
...
async function handleBreak(channelId: ChannelGuid): Promise<void> {
