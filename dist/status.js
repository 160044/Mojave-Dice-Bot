...
exports.StatusTracker = exports.DND_CONDITIONS = void 0;
/**
 * D&D 5e Conditions with descriptions
 */
exports.DND_CONDITIONS = {
...
class StatusTracker {
     ...
     */
    addStatus(channelId, combatantName, condition, duration) {
        ...
        const conditionData = exports.DND_CONDITIONS[conditionLower];
        if (!conditionData) {
        ...
        const existing = channelEffects[combatantName].find(e => e.condition.toLowerCase() === conditionLower);
        if (existing) {
     ...
     */
    removeStatus(channelId, combatantName, condition) {
        const channelEffects = this.effects.get(channelId);
        if (!channelEffects || !channelEffects[combatantName])
            return false;
        if (condition) {
            // Remove specific condition
            const conditionLower = condition.toLowerCase();
            const index = channelEffects[combatantName].findIndex(e => e.condition.toLowerCase() === conditionLower);
            if (index === -1)
                return false;
     ...
     */
    getStatuses(channelId, combatantName) {
        const channelEffects = this.effects.get(channelId);
        if (!channelEffects)
            return [];
     ...
     */
    formatStatuses(channelId, combatantName) {
        const channelEffects = this.effects.get(channelId);
        if (!channelEffects) {
     ...
     */
    getEmojiSummary(channelId, combatantName) {
    ...
    importData(data) {
        if (!data)
            return;
        this.effects.clear();
        Object.entries(data).forEach(([channelId, channelEffects]) => {
